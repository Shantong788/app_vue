...
export default {
  ...
  methods: {
    ...
    getBanner() {
      this.axios
        .post(this.getApi("web/ad_position"), `type=2`)
        .then(res => {
          // 关闭load
    ...
    // 获取主页分类图标
    getClassifyIcon() {
      this.axios.post(this.getApi("web/nav")).then(res => {
        if (res.data.code === 0) {
    ...
    // 获取商城快讯数据
    getNotice() {
      this.axios.post(this.getApi("web/notice")).then(response => {
    ...
    // 获取热销商品
    getHotGoods() {
      this.axios.post(this.getApi("web/hot_sale")).then(res => {
        if (res.data.code === 0) {
    ...
    // 获取广告条
    getAdvert() {
      this.axios.post(this.getApi("web/ad_position"), `type=7`).then(res => {
        if (res.data.code === 0) {
          if (res.data.list.length > 0) {
    ...
    getGoods() {
      this.axios
        .post(
          this.getApi("web/guess_you_like"),
          this.dafa({
            type: 0,
            num: 20,
    ...
    // 获取限时抢购
    getFalshGoods() {
      this.axios.post(this.getApi("web/flash_sale")).then(res => {
        if (res.data.code === 0) {
    ...
    getModuleGoods() {
      this.axios
        .post(this.getApi("web/floor_sale"))
        .then(res => {
          if (res.data.code === 0) {
...
.g-jing_xuan li:nth-child(4),
.g-jing_xuan li:nth-child(8) {
  margin-right: 0;
}
